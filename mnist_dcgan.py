...
def gen_noise():
    return torch.randn((5 * 5), 100).view(-1, 100).type(dtype)
...
if not os.path.isdir('DCGAN_MNIST/Random_results'):
    os.mkdir('DCGAN_MNIST/Random_results')
if not os.path.isdir('DCGAN_MNIST/Fixed_results'):
    os.mkdir('DCGAN_MNIST/Fixed_results')
...
for epoch in range(train_epoch):
    ...
    p = 'DCGAN_MNIST/Random_results/MNIST_DCGAN_' + str(epoch + 1) + '.png'
    fixed_p = 'DCGAN_MNIST/Fixed_results/MNIST_DCGAN_' + str(epoch + 1) + '.png'
    ...
    if epoch % 2 == 0:
        torch.save(G.state_dict(), "DCGAN_MNIST/generator_param.pkl")
        torch.save(D.state_dict(), "DCGAN_MNIST/discriminator_param.pkl") # for safety!
...
torch.save(G.state_dict(), "DCGAN_MNIST/generator_param.pkl")
torch.save(D.state_dict(), "DCGAN_MNIST/discriminator_param.pkl")
with open('DCGAN_MNIST/train_hist.pkl', 'wb') as f:
    pickle.dump(train_hist, f)

show_train_hist(train_hist, save=True, path='DCGAN_MNIST/MNIST_DCGAN_train_hist.png')

# images = []
# for e in range(train_epoch):
#     img_name = 'DCGAN_MNIST/Fixed_results/MNIST_DCGAN_' + str(e + 1) + '.png'
